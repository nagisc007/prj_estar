...
def sc_darkhome(w: wd.World):
    ...
    return w.scene("真っ暗な家の中で",
            ...
            h.deal("小さく躓いた$n_goroはほとんど消えかけた街灯を見上げ",
                ...
                "スマートフォンを取り出す。",
                "その明かりの方が余程頼れたが",
                "早めに帰ると言ったのに既に時刻は十一時を回ってしまっている"),
            h.move("疲れた足を動かして玄関に急ぐ。",
                "彼女が", "闇の世界で待っているはずだから"),
            goro.talk("ただいま"),
            h.deal("鍵を開けて自宅に入る。",
...
def sc_alwayshelp(w: wd.World):
    ...
    return w.scene("いつでも助けたい",
            ...
            h.deal("植え込みのブロックに横並びで腰を下ろし",
                    ...
                    "$Sは$ln_lunaとの出会いも似たようなものだったことを思い出し", "口元に笑みを浮かべた"),
...
def sc_alonegoout(w: wd.World):
    ...
    return w.scene("孤独な外出",
            ...
            goro.talk("明日明後日は演奏会があるんだったね。",
                    "今日はもう寝た方がいい"),
            h.deal("$Sがそう言うと",
                    "何も言わずに彼女は自室の方へと戻っていってしまった"),
            goro.talk("おやすみ"),
            w.tag.symbol("◆"),
            h.deal("翌朝",
                    "彼女を迎えに楽団のメンバーが訪れた"),
            h.deal("既に何度か顔を合わせているので全く知らない人間ということはないが",
                    "バンを運転してきた$fn_takahashiという男性がやたらと親しげに彼女に話しかけながら",
                    "後部座席へと案内している"),
            h.think("今の楽団に入ってからまだ三ヶ月と新人な方だというが",
                    "周りの人たちとのやり取りを見ていると普段は物静かな彼女がよく受け答えして笑っている"),
            h.think("最初の頃はああいう笑顔にしてやりたくて色々と苦労を重ねたことを思い出し",
                    "$Sは苦笑した"),
            goro.talk("あの……$ln_lunaのこと", "宜しくお願いします"),
            w.takahashi.talk("大丈夫ですよ", "$fn_lunaさんは。", "いつも一人で大半のことをやってしまうんですから"),
            h.deal("そう言って$Sをじっと見やった$fn_takahashiは",
                    "運転席に座り", "エンジンを掛けた。",
...
def sc_worldtone(w: wd.World):
    ...
    return w.scene("音の世界",
            ...
            h.deal("最初の方は謝罪や$Sの本音を交えた気持ちを文章にして送ってみたが",
                "そこには「そうじゃない」が返ってくるだけで二の句には「」と書かれた"),
            ...
            h.deal("丸テーブルの上に一枚のＣＤが置かれているのに気づき",
                "$Sはそれを手にする。",
                "ケースには手書きでこの前の公演日と『$ln_lunaさんへ』とマジックで書かれていた"),
            h.deal("あの$fn_takahashiとかいう若者の顔が浮かんだけれど",
...
def sc_lookmoon(w: wd.World):
    ...
    return w.scene("月を見る、月を聞く",
            h.be(w.stage.onstreet),
            h.look("そこには$fn_takahashiに連れられて外に出てきた$ln_lunaがいた"),
            goro.talk("$luna！"),
            h.deal("$Sの呼びかけに対して彼女は「そうじゃない」とは言わずにただ黙って歩いてくる"),
            h.deal("$fn_takahashiは彼女が$Sの前まで無事に歩いて辿り着いたのを見届けると",
                "深くお辞儀をして彼の家へと帰っていった"),
            goro.talk("$luna？"),
            luna.talk("それでいい"),
            goro.talk("え？"),
