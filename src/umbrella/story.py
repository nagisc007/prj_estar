...
def sc_mysteryousman(w: wd.World):
    ...
    return w.scene("謎の男性",
            ...
            miyu.think().d("最初に$senpaiに連れて来られたからだ",
                "とは言えなかった"),
            miyu.look().d("向かいのビルの中程の一つの", "窓の明かりが消える"),
...
def sc_lookback(w: wd.World):
    ...
    return w.scene("私の回想",
            ...
            miyu.think().d("$meはもう三ヶ月も時間が経ってしまった出来事を",
                "記憶の片隅からロードする"),
            ...
            miyu.talk().t("その紫陽花の先輩を$Mとして話しますね"),
            miyu.look().d("確認するように男性を見たが", "ほとんど顎を動かさずに頷いて続きを促した"),
            miyu.talk().t("$Mは職場に配属されて間もなかった$meの指導係でした。",
                "会社では必ずより経験年数の多い者がその役割を担うんですが",
                "どういう訳か$Mは$meより二ヶ月だけ余分に経験値があるだけの",
...
def sc_goodbye(w: wd.World):
    ...
    return w.scene("先輩とサヨナラ",
            ...
            miyu.look().d("仕事が定時で終わった日に",
                ...
                "鍵の在り処は$senpaiだけが知っていたから入れないだろうと思っていたけれど",
                "夕陽がすっかりビルの間に沈んでしまった暗がりのドアは",
                "静かに開いた"),
            miyu.look().d("あの日二度目に見た宵の光の雨を",
                "$meは二度と記憶から消去することは出来ないだろう"),
            miyu.look().d("傘越しには見えないけれど",
                "今",
                "目の前で目線を同じにしてくれている男性に降り注いでいる光よりもずっと",
                "あの日は沢山のそれが落ちてきていた。",
                "闇の帳に小さな点が映り",
